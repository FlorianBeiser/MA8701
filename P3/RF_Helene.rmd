...
Therewith, we end up a data set with a time index, one `water_temp` measurement, and 16 atmospheric measurements, where we want to do prediction of the water temperature (what will be explained in a bit more detail in the subsequent sections).

## Model Building and Preprocessing 

Our goal is to predict the water temperature at the next time using all information from the previous time - this includes the `water_temp` at that previous time. Therefore, we introduce all atmospheric observations \emph{and} the water temperature at time $t$ as covariates and what to model the response, which is the water temperature at time $t+1$. 
