...
As seen before, we cannot afford a split into test and training set for our data set, therefore we use bootstrapping for inference. Bootstrap can be applied here to find the proportion of times each covariate is shrinked to zero.  So it is a way of validation. By looking at the boxplot and barplot, we observe the percentage of time each variable is shrinked to zero.

@YG: Please describe shortly what is depicted in the barplot (just calling it "barplot" is not really informative for the reader, but you can assume a lot of preknowledge such that is done in one sentence). Same applies to boxplot.


```{r,echo = F, eval=T, tidy = F, fig.width=16, fig.height=8, fig.cap="\\label{fig:barplot} @YG: Please provide still a caption and not just a label"}
...
for (b in 1:B)
{
  ...
  regrp=grplasso(x=x_mod_group, y = y_mod, index = index_mod, lambda = grp_lambda_mod[-(1:3)], model = LinReg(),
...
From the barplot \ref{fig:barplot} one can tell that the majority of the coefficients for those year covariates `bj` and those factorical covariates `bez` are mainly zero while the dominating factors are non zero which reflects their significance as well. 

@YG: I put the description of the results after the displaying of the results to have a more logical order.


```{r, echo = F, eval=T, tidy = F, fig.width=15, fig.height=5, fig.cap="\\label{fig:boxplot} @YG: Please provide still a caption and not just a label"}
...
@YG: Inference should evaluate the goodness of the prediction (constructed with the group lasso) and data split is just a technique. So from bootstrapping we should get an outcome on the fit of our model and not on the techniques to use
