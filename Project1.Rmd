...
## Ridge
```{r ridge, eval = TRUE, echo = TRUE, fig.align="center", fig.cap="\\label{fig:ridge_coeff}Coefficient path along lambda variation"}
start <- glmnet(x = x_mod, y = y_mod, standardize=FALSE, alpha = 0)
autolambda <- start$lambda
newlambda <- c(autolambda, 10, 5, 3, 1, 0.5, 0.1) # add more to approach zero lambda
ridge_fit <- glmnet(x_mod, y_mod, standardize=FALSE, alpha = 0, lambda = newlambda)
# plot(ridge_fit, xvar = "lambda", label = T)
cv.ridge <- cv.glmnet(x_mod, y_mod, standardize=FALSE, alpha = 0, lambda = newlambda)
