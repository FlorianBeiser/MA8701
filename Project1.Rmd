...
```{r groupLasso, eval=TRUE, echo=FALSE, fig.align="center", fig.height=8, fig.width=8}
...
```{r final, eval=TRUE, echo=FALSE, fig.height=5, fig.width=15}
...
results_mod=cbind(coef(lm_mod), coef(ridge_fit,s=cv.ridge$lambda.1se), coef(fit.lasso,s=cv.lasso$lambda.1se), coef(fin_mod))
...
```{r,eval=T, tidy = F, fig.width=15, fig.height=5}
# boostrap loop
set.seed(2021)
B=10
n=nrow(x_mod)
...
lassopp=ggplot(lassods,aes(x=Var2,y=value))+
  geom_boxplot()+
  ggtitle("Boxplots for boostrapped lasso for diabetes data")
# lassopp
...
ridgepp=ggplot(ridgeds,aes(x=Var2,y=value))+
  geom_boxplot()+
  ggtitle("Boxplots for boostrapped ridge for diabetes data")
# ridgepp
