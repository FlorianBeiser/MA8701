...
As seen before, we cannot afford a split into test and training set for our data set, therefore we use bootstrapping for inference. Bootstrap can be applied here to find the proportion of times each covariate is shrinked to zero.  So it is a way of validation. Barplot demonstrates how many percentages of that coefficient being zero inside the bootstrap loop, while the boxplot gives the variability view on how dispersed or concentrated that coefficient can be.

```{r,echo = F, eval=T, tidy = F, fig.width=16, fig.height=4, fig.cap="\\label{fig:barplot} Barplot of the bootstrapped coefficients"}
...
for (b in 1:B)
{
  ...
  regrp=gglasso(x = x_mod_group[,-1], y = y_mod, group = index_mod[-1], weight = wids, loss = "ls",lambda=exp(cvfitls_mod$lambda.1se)) # gglasso
  # regrp=grplasso(x=x_mod_group, y = y_mod, index = index_mod, weights = wids, lambda = grp_lambda_mod[-(1:3)], model = LinReg(),
  #                            penscale = sqrt, control = grpl.control(update.hess = "lambda", trace = 0))
  
  # regrp=grplasso(x = x_mod_group[,-1], y = y_mod, group = index_mod[-1], loss = "ls",lambda=exp(cvfitls_mod$lambda.1se))
  lassomat[b,]=as.vector(coef(resl)) 
  ridgemat[b,]=as.vector(coef(resr)) 
  grplassomat[b,]=as.vector()
  regrp_coef = coef(regrp) # it gives all the coefficients given the lambda grid
  
  lambda_regrp = as.numeric(colnames(regrp_coef)) # find the corresponding element in the lambda grid
  index_lambda1se = which.min(abs(lambda_regrp - cvfitls_mod$lambda.1se)) # only use 1se lambda for validation
  grplassomat[b,]=as.vector(regrp_coef[,index_lambda1se]) # it always give the same coefficients? why this happened
  # print(as.vector(regrp_coef[,index_lambda1se]))
}
...
# grplasso0perc=apply(abs(grplassomat[,-1])<.Machine$double.eps,2,mean)
# barplot(grplasso0perc) # only use oridinary lasso here
lasso0perc=apply(abs(lassomat[,-1])<.Machine$double.eps,2,mean)
barplot(lasso0perc) # only use oridinary lasso here
```

From the barplot \ref{fig:barplot} one can tell that after the grouplasso implementation, the majority of the coefficients for those year covariates `bj` and those factorical covariates `bez` are not remaining zero since they are well considered in the group lasso already. 

```{r, echo = F, eval=T, tidy = F, fig.width=15, fig.height=5, fig.cap="\\label{fig:boxplot} Boxplot of the bootstrapped coefficients"}
...
The boxplot \ref{fig:boxplot} reveals the variability of those covariates that used to be zero. It also demonstrates the efficacy of using grouplasso for considering such conditions where many factorial variables are present in the dataset, normal lasso or ridge methods tend to ignore them by shrinking them to zeros. However, the grouplasso takes into account for their significance as well. The bootstrapped boxplot does prove that the variability of some of coefficients are huge which might imply that the size of the dataset is too small to afford a test set. 
